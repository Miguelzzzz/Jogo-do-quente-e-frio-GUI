...
class Game:
    """Classe responsável pela logica do jogo."""

    def __init__(self, player_name="Jogador", digits=1):
        self.player_name = player_name
        self.digits = digits
        self.minimum = 10 ** (digits - 1) if digits > 1 else 0
        self.maximum = (10 ** digits) - 1
        self.mysterious_number = random.randint(self.minimum, self.maximum)
        self.attempts = [] 
        self.counter = 0
    ...
    def guess(self, value: int):
        ...
        if value == self.mysterious_number:
            return "acertou"
        elif value < self.mysterious_number:
            return "menor"
        else:
            return "maior"
    ...
    def reset(self, digits: int = None):
        ...
        if digits is not None: self.digits = digits
